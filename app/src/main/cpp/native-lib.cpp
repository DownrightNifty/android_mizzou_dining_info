...
#define dlog(...) __android_log_print(ANDROID_LOG_DEBUG, "native-lib", __VA_ARGS__)
...
std::vector<Location> GetScheduleData(const std::string& date, bool debugMode, std::string cachedHtmlPath) {
    ...
    if (debugMode) {
        ...
        if (file.is_open()) {
            ...
            dlog("Failed to open cached file.\n");
            return locations;
        }
    } else {
        ...
        if (res && res->status == 200) {
            ...
            dlog("Error fetching data from Mizzou website.\n");
            return locations;
        }
    }
...
Java_com_example_myapplication_MainActivity_getScheduleData(
    ...
    // Print information
    if (locations.empty()) {
        dlog("Locations vector is empty.\n");
    }
    else {
